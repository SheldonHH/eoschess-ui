...
<v-layout row wrap>
    <v-flex xs4>
        <v-card class="addApi text-xs-center">
            ...
            ]" item-value="value" v-model="addEndpointProtocol" label="Protocol" single-line bottom dense required></v-select>
    ...
    <v-flex xs8>
        <v-data-table v-bind:headers="endpointListHeaders" :items="getEndpoints" no-data-text="No endpoints available" hide-actions class="elevation-1">
            <template slot="items" slot-scope="props">
                ...
                <v-btn v-else-if="!getCurrentEndpoint || getCurrentEndpoint.url !== props.item.url" color="primary" @click="useEndpoint(props.item.url)">Use</v-btn>
                <v-btn v-else-if="getCurrentEndpoint && getCurrentEndpoint.url === props.item.url"  @click="disconnectEndpoint(props.item.url)">Disconnect</v-btn>
            </template>
        </v-data-table>
    </v-flex>
